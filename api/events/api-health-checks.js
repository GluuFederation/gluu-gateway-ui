...
module.exports = {
    ...
    start : function(hc) {
        ...
        sails.log(new Date(), 'Start scheduled health checks for hc ', hc.id);
        var self = this;
    ...
    stop : function(hc) {
        if(tasks[hc.id]) {
            sails.log(new Date(), 'Stopping health check for hc ' + hc.id);
    ...
    createCron : function(hc) {
        ...
        return cron.schedule('* * * * *', function() {
            sails.log(new Date(), 'Performing health check for hc ' + hc.id);
            ...
            sails.models.apihealthcheck.findOne({
                ...
                if(err || !hc) {
                    ...
                    sails.log(new Date(), "api_health_checks => Failed to retrieve apiHealthCheck with id " + hc.id,err)

                }else{
                    tasks[hc.id] = _.merge(tasks[hc.id],hc.data)

                    if(!hc.health_check_endpoint) {
                        sails.log(new Date(), "api_health_checks =>","no health_check_endpoint defined. Ending it.")
                        return false;
                    }

                    sails.log(new Date(), "api_health_checks => Performing GET request to " + hc.health_check_endpoint)
                    ...
                    unirest.get(hc.health_check_endpoint)
                        .end(function (response) {
                            if (response.error)  { // health check failed
                                ...
                                sails.log(new Date(), 'Health check for hc ' + hc.id + ' succeeded');
    ...
    updatehcHealthCheckDetails : function(hcId) {
        ...
        sails.models.apihealthcheck.update({id: hcId},{
            ...
            if(err) {
                sails.log(new Date(), "health_checks:updatehcHealthCheckDetails:failed",err)
            }else{
    ...
    notifyNotificationEndpoint : function(hc) {
        ...
        sails.log(new Date(), "api_health_checks:notifyNotificationEndpoint")
        if(!hc.notification_endpoint) {
            sails.log(new Date(), "api_health_checks:notifyNotificationEndpoint => No notification endpoint defined")
            return false;
        }
        ...
        unirest.post(hc.notification_endpoint)
            ...
            .end(function (response) {
                if (response.error)  {
                    sails.log(new Date(), "api_health_checks:notifyNotificationEndpoint => Failed to notify notification endpoint",response.error)
                }else{
                    sails.log(new Date(), "api_health_checks:notifyNotificationEndpoint => Succeeded to notify notification endpoint")
                }
